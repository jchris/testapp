...
export default function App() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (databaseName.trim()) {
      ...
      const updatedHistory = [newDbName, ...dbHistory.filter(db => db !== newDbName)].slice(0, 5);
      setDbHistory(updatedHistory);
      localStorage.setItem("fireproof-db-history", JSON.stringify(updatedHistory));
      ...
      <div style={{ marginBottom: 24, padding: 16, backgroundColor: "#f8f9fa", borderRadius: 6 }}>
        <form onSubmit={handleSubmit} style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <input
            ...
            style={{ flex: 1, padding: 6 }}
          />
          <button type="submit" style={{ padding: "6px 18px", cursor: "pointer" }}>Switch</button>
        </form>
        
        {dbHistory.length > 0 && (
          <div>
            <div style={{ fontSize: "90%", color: "#666", marginBottom: 8 }}>Recent databases:</div>
